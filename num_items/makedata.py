...
class MakeSingleDataSet(MakeDataSetBase):
    ...
    def __init__(self, root_dir, image_size_w, image_size_h):
        ...
        self.labeldata = pd.read_csv('num_items.csv') #ラベルのデータ
        self.categories = [1,2,3,4] # カテゴリー名
        self.categories_dict = {1:0, 2:1, 3:2, 4:3}

    def getLabel(self, filename):
        # labeldataから当該ファイルのラベルを検索して返す
        df = self.labeldata
        label = df['num_items'][df.file_name == filename[:-8] + ".jpg"] # ファイル名の前半部分からラベルを検索
...
#                continue
        X = np.array(X)
        Y = np.array(Y)
        np.save("./num_items/dataX_num_items.npy", X) #分割したデータを保存
        np.save("./num_items/dataY_num_items.npy", Y) #分割したデータを保存
        return (X,Y)
